...
export const Example = () => {
  ...
  >('bottom');
  const [slidePositionDrawer, setSlidePositionDrawer] = useState<
    'bottom' | 'top' | 'left' | 'right'
  >('right');
  const [contentPositionDrawer, setContentPositionDrawer] = useState<
    'bottom' | 'top' | 'left' | 'right'
  >('right');
  ...
  const onTogglePopup = (
    slidePosition: 'bottom' | 'top' | 'left' | 'right' | 'center' | null,
    contentPosition: 'bottom' | 'top' | 'center' | 'left' | 'right' | null
  ) => {
  ...
  const onToggle = (
    slidePosition: 'bottom' | 'top' | 'left' | 'right' | null,
    contentPosition: 'bottom' | 'top' | 'left' | 'right' | null
  ) => {
  ...
  const generateRandomRows = (
    ...
    return Array.from({ length: count }, (_, index) => ({
      ...
      id: index + 1,
      ...(isColumnId
        ? { column_id: Math.floor(Math.random() * (maxColumn - minColumn + 1)) + minColumn }
        : {})
    }));
  };
    ...
    >
      ...
      <TagCommon gap={SPACES.l}>
        ...
        <TagCommon fd='row' gap={SPACES.l}>
          ...
          <Button.AsNavLink to={APP_KEYS.ROUTER_KEYS.HOME} content='navigation button ' />
        </TagCommon>
      </TagCommon>
      <Table
        className='scroll'
        ...
        {({ values, setFieldValue }) => (
          <Form>
            ...
            <TagCommon fd='row' gap={SPACES.l} ai='flex-end' margin='0 0 2rem 0'>
              ...
              />
              <Calendar label='Calendar (Date[])' name='calendar_multiple' mode='multiple' />
            </TagCommon>
            TEXTAREA
            ...
            <TextArea name='resizable' rows={2} label='TextArea resizable' resizable />
            DRAWER
            <TagCommon fd='row' gap='2rem' margin='0 0 2rem 0'>
              <Button
                content='left'
                variant='primary'
                onClick={onToggle.bind(this, 'left', 'left')}
              />
              <Button
                content='right'
                variant='primary'
                onClick={onToggle.bind(this, 'right', 'right')}
              />
              <Button
                content='top'
                variant='primary'
                onClick={onToggle.bind(this, 'top', 'left')}
              />
              <Button
                content='bottom'
                variant='primary'
      ...
      <TagCommon fd='row' gap={SPACES.xxxxxxl_}>
        <Accordion visibleBlock='synchronized 1' name='synchronized'>
          d,cdlcldcdmckdcmkdcmkcdmkcdkcdk
        </Accordion>

        <Accordion name='synchronized' visibleBlock='synchronized 2'>
          d,cdlcldcdmckdcmkdcmkcdmkcdkcdk
        </Accordion>

        <Accordion name='test2' visibleBlock='dmcdcmkm'>
          d,cdlcldcdmckdcmkdcmkcdmkcdkcdk
        </Accordion>
      </TagCommon>
      DRAGANDDROP
      ...
      <Styled.Net>
        {[1, 2, 3, 4].map((v) => (
          <TagCommon key={v} className='item' background='red'>
            {v}
          </TagCommon>
        ))}
      </Styled.Net>
